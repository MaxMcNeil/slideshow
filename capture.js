...
// Splits a card's raw text into TWO distinct, non-overlapping pieces of
// display copy instead of one string reused everywhere:
//   - title:   short, shown above the image (the popup) and in the ticker.
//   - summary: longer, shown on the dedicated summary screen after the image.
// Neither is a copy of the raw headline (rawLines[1] is deliberately
// skipped) and the two no longer duplicate each other's text — the title
// is the excerpt's first sentence (prefixed by the section/category), the
// summary is whatever sentences follow it. This is still a local, rule-based
// restructuring (split by sentence), not a true AI paraphrase — there is no
// external API call here — but it guarantees the two on-screen texts are
// never identical, and that the title is a short, distinct headline rather
// than the full excerpt dumped verbatim above the image.
...
function extractCaption(rawInnerText, sourceLabel) {
    ...
    if (rawLines.length === 0) return { title: '', summary: '', date, source: sourceLabel };

    const category = rawLines[0].replace(/•/g, '—').trim();
    // rawLines[1] is normally the headline — deliberately not used as the
    // lead. The remaining lines (the descriptive excerpt) are what gets
    // split between title and summary below.
    const excerptLines = rawLines.length > 2 ? rawLines.slice(2) : rawLines.slice(1);
    const excerpt = excerptLines.join(' ').replace(/\s+/g, ' ').trim();

    if (!excerpt) return { title: category, summary: '', date, source: sourceLabel };

    const sentences = splitIntoSentences(excerpt);
    let lead = (sentences[0] || excerpt).trim();
    if (lead.length > 100) lead = lead.slice(0, 100).trim() + '…';
    let title = category ? `${category} — ${lead}` : lead;
    if (title.length > 140) title = title.slice(0, 140).trim() + '…';

    let summary = sentences.length > 1 ? sentences.slice(1).join(' ').trim() : excerpt;
    if (!summary) summary = excerpt;
    if (summary.length > 380) summary = summary.slice(0, 380).trim() + '…';

    return { title, summary, date, source: sourceLabel };
}
