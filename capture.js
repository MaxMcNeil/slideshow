...
function extractCaption(rawText) {
    ...
    const dateMatch = text.match(/publié le\s+\d{1,2}\s+[a-zéûôîàè]+\s+\d{4}/i)
        || text.match(/\ble\s+\d{1,2}\s+[a-zéûôîàè]+\s+\d{4}/i);
    let date = '';
    if (dateMatch) {
        date = dateMatch[0].replace(/^publié le\s+/i, '').trim();
        text = (text.slice(0, dateMatch.index) + text.slice(dateMatch.index + dateMatch[0].length)).trim();
    }
    if (text.length > 380) text = text.slice(0, 380).trim() + '…';
    return { body: text, date };
}
        ...
        try {
            ...
            for (let i = 0; i < elements.length; i++) {
                try {
                    ...
                    cardCaptions[count] = extractCaption(rawText);
