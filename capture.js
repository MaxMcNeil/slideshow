...
//          fell back to showing just "CNews". reformulateTitles() below is
//          what turns the raw headline captured here into that reworded
//          title, for every card without exception.)
// summary: the genuine descriptive text that follows the headline, if any.
//          Guaranteed non-empty — cards with no text beyond their headline
//          (nothing else was available to scrape) fall back to the
...
function extractCaption(rawInnerText, sourceLabel) {
    ...
    // source material for reformulateTitles(), never surfaced verbatim.
    ...
    // title/summary-fallback are filled in later by reformulateTitles(),
    // once every card on every source has been collected (so titles can be
    // reformulated in one batch instead of one call per card).
    return { headline, category, summary, hasRealSummary, date, source: sourceLabel };
}

// ---------- title reformulation (every card, always reworded, never copied) ----------
...
// bare category or source label the way the old metadata-only title did.
// Uses the Anthropic API for genuine paraphrasing when ANTHROPIC_API_KEY is
// set (see .github/workflows/updates.yml); otherwise falls back to a local,
// dependency-free reword so the pipeline still produces a real title (never
// empty, never source-name-only) even without a key configured.
async function reformulateTitles(captions) {
    const withHeadline = captions
        .map((c, i) => ({ i, headline: c && c.headline ? c.headline : '' }))
        .filter(x => x.headline);
    ...
    if (apiKey) {
        try {
            apiResults = await callAnthropicForTitles(withHeadline, apiKey);
        } catch (e) {
            console.warn(`  ⚠ Reformulation IA indisponible, repli local : ${e.message}`);
        }
    } else {
        console.log('  ℹ ANTHROPIC_API_KEY absente — reformulation locale (repli) utilisée pour tous les titres');
    }

    for (const { i, headline } of withHeadline) {
        const c = captions[i];
        const title = apiResults[i] || localParaphrase(headline, c.category);
        c.title = title;
        if (!c.hasRealSummary) c.summary = title;
        delete c.headline;
        delete c.category;
        delete c.hasRealSummary;
    }
}

async function callAnthropicForTitles(items, apiKey) {
    const axios = require('axios');
    const list = items.map(x => `${x.i}: ${x.headline}`).join('\n');
    const prompt =
        `Voici des titres bruts d'articles de presse française, un par ligne au format "index: titre".\n` +
        `Pour CHAQUE ligne, écris un titre court reformulé en français : mêmes faits, formulation ` +
        `différente (pas une copie ni une simple troncature du titre original), neutre, factuel, ` +
        `70 caractères maximum.\n` +
        `Réponds UNIQUEMENT avec un objet JSON de la forme {"0": "...", "1": "...", ...}, sans texte ` +
        `avant ni après, sans balises de code.\n\n${list}`;
    ...
    for (const { i } of items) {
        if (typeof parsed[String(i)] === 'string' && parsed[String(i)].trim()) {
            out[i] = parsed[String(i)].trim();
        }
    }
    return out;
}

// Dependency-free reword used only when the API is unavailable (no key, or
// the call failed). Strips leading filler words and prefixes a short tag so
// the result reads as reworded rather than a raw/truncated copy — lower
// quality than the AI path, but guarantees no card is ever left with an
// empty or source-name-only title.
function localParaphrase(headline, category) {
...
async function main() {
    ...
    console.log('\n🖋 Reformulation des titres (toutes les cartes)...');
    await reformulateTitles(cardCaptions);
