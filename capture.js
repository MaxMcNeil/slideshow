...
// Builds the on-screen caption as a genuine restructuring of the article
// text — NOT a copy of the raw headline. It leads with the descriptive
// excerpt (prefixed by the category) and deliberately drops the punchy
// title line. This reduces direct copying, though it is a structural
// reformatting, not a true AI paraphrase — the wording within the excerpt
// itself still comes from the source.
function extractCaption(rawInnerText, sourceLabel) {
    ...
    if (rawLines.length === 0) return { body: '', date, source: sourceLabel };

    const category = rawLines[0].replace(/•/g, '—').trim();
    // rawLines[1] is normally the headline — deliberately not used as the
    // lead. The remaining lines (the descriptive excerpt) become the
    // caption body instead.
    const excerptLines = rawLines.length > 2 ? rawLines.slice(2) : rawLines.slice(1);
    let excerpt = excerptLines.join(' ').replace(/\s+/g, ' ').trim();

    let body = excerpt ? `${category} : ${excerpt}` : category;
    if (body.length > 380) body = body.slice(0, 380).trim() + '…';
    return { body, date, source: sourceLabel };
}
