...
// title:   shown above the image (popup) and in the ticker. Built ONLY from
//          metadata (category / source / date) — never from article
//          wording — so it is structurally impossible for it to repeat the
//          headline that's already legible on the card screenshot itself.
//          (An earlier version tried to reuse the excerpt's first sentence
//          as a "short headline", but on sites where the headline and
//          excerpt share one text node with no line break, that first
//          sentence WAS the headline, so the popup ended up echoing the
//          image verbatim — exactly what this is meant to avoid.)
...
//          (nothing else was available to scrape) still get a fallback
//          message so the summary screen appears after every single card,
//          not only the ones with a long excerpt.
function extractCaption(rawInnerText, sourceLabel) {
    ...
    // First sentence of fullText is the headline (already fully legible in
    // the card screenshot) — drop it unconditionally. Anything after it is
    // genuine, not-shown-elsewhere descriptive text.
    const sentences = splitIntoSentences(fullText);
    let summary = sentences.length > 1 ? sentences.slice(1).join(' ').trim() : '';
    if (summary.length > 380) summary = summary.slice(0, 380).trim() + '…';
    if (!summary) summary = 'Pas de résumé disponible au-delà du titre pour cette dépêche.';

    const title = category || sourceLabel || 'Dépêche';
